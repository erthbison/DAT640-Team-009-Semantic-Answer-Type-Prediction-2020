...
def get_types(filename: str) -> Dict[str, OntologyType]:
	...
	Args:
	----
		filename (str): Path to the file containing the types

	Returns:
	-------
		Dict[str, OntologyType]: Dictionary storing type name and OntologyType objects for the types
	"""
...
def get_entities(filename, types: Dict[str, OntologyType]) -> Dict[str, Entity]:
	...
	Args:
	----
		filename (str): Path to the file containing the entities
		types (Dict[str, OntologyType]): Dictionary storing type name and OntologyType objects for the types

	Returns:
	-------
		Dict[str, Entity]: Dictionary storing entity name and Entity objects for the entities
	"""
...
def extract_literal(entity_repr: Dict[str, str], filename: str) -> None:
	"""
	Extracts literal value from file and adds it to the the entity representation of its respective
	entity

	Args:
	----
		entity_repr (Dict[str, str]): Dictionary storing entity name and entity representation for the entities
		filename (str): Path to the file containing the literals
	"""
...
def extract_uri(entity_repr: Dict[str, str], filename: str) -> None:
	"""
	...
	----
		entity_repr (Dict[str, str]): Dictionary storing entity name and entity representation for the entities
		filename (str): Path to the file containing the uris
	"""
	with open(filename, "r", encoding="utf-8") as f:
		f.__next__()
		for i, line in enumerate(f):
...
def strip_ontology_url(url) -> str:
	...
	Args:
	----
		url (str): The uri of the ontology element

	Returns:
	-------
		str: The stripped uri
	"""
	url = url.strip("<>")
...
def strip_dbpedia_url(url: str, type:str) -> str:
	"""
	Strips the uri of a dbpedia element

	Args:
	----
		url (str): The uri of the dbpedia element
		type (str): The type of the dbpedia element
