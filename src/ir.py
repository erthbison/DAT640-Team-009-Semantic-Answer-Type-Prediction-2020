...
class EntityCentric():
	...
	def Score(self, query: Iterable[str]) -> List[Tuple[float, OntologyType]]:
		...
		type_scores = np.fromiter(map(lambda x: self.score_type(
			x, top_entities, top_entity_scores), self.types), np.float64)

		# Find the types with the num_types highest scores and retrieve the score and types
		top_indices = np.argpartition(
			type_scores, -self.num_types)[-self.num_types:]
	...
	def score_type(self, type, top_entities, top_entity_score):
		...
		"""
		weights = np.fromiter(
			map(self.weight, top_entities, [type] * self.k), np.float64)
